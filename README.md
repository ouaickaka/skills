...
Use the harness ask tool to learn which plugins the user wants and which harness they use. Claude Code uses AskUserQuestion, OMP uses ask, OpenCode uses question (needs permission.question: "allow" in opencode.json; fall back to text if unavailable).
